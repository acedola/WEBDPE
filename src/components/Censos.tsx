...
const Censos: React.FC<CensosProps> = ({ option }) => {
  ...
  useEffect(() => {
    async function fetchSMenu() {
      if (option !== null) {
        try {
          console.log(`el option que pasa el censo es ${option}`)
  ...
  return (
    <div className="flex justify-center w-full">
      <div className="grid grid-cols-4 gap-10 w-3/5 mt-12 mb-40 estiloCenso">
        {lists.length > 0 ? (
          lists.map((censo: CensosData) => (
            ...
            >
              <div className="flex items-center justify-center">
                <Link
                  href={`/censos/${option}/secciones/${censo.attributes.cat_nombre}`}
                >
                  <img
                    className="w-40 h-32"
