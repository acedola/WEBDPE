...
import styles from '@/app/styles/articulosCenso.module.css';
...
interface ArticuloData {
  ...
  attributes: {
    ...
    TituloCuadro: string;
    informes: string; // Cambiado de 'Informes' a 'indformes' para coincidir con tu JSON
  ...
  useEffect(() => {
    async function fetchSecc() {
      if (option !== null) {
        ...
        const data = await loadArticulosCensos({ cat_nombre: option });
        if (data) {
          setLists(data.data);
        }
      }
        ...
        <div className="flex flex-col items-center py-8">
          ...
          {lists[0].attributes.contenido.data !== null && (
            <img
              ...
              className="py-8 mb-20 rounded-lg shadow-xl dark:shadow-gray-800 imgArtCenso"
              alt="imagen"
            />
          )}
          <p
          ...
          {lists[0].attributes.TituloInforme && (
            <>
              <div className="w-full bg-cyan-400 px-2 py-2 mb-4 font-sans leading-none tracking-tight text-gray-900 dark:text-white">
               ...
               <div
  className={`mb-3 w-3/5 text-gray-500 ${styles['contenedor-informes']}`}
  dangerouslySetInnerHTML={{
